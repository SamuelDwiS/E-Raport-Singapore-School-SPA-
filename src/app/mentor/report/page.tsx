...
function scoreColor(val: string): string {
  ...
  if (isNaN(v) || val === "") return "border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100";
      ...
      {view === "list" ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-gray-700 overflow-hidden">
          <div className="p-4 sm:p-6 border-b border-gray-50 dark:border-gray-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gray-50/30 dark:bg-gray-800/50">
            ...
            <div className="flex flex-wrap gap-2 w-full md:w-auto">
               <select className="flex-1 md:flex-none text-xs font-bold border-none bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg px-3 py-2 outline-none" value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)}>
                  {CLASSES.map(c => <option key={c} value={c}>{c}</option>)}
               </select>
               <select className="flex-1 md:flex-none text-xs font-bold border-none bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg px-3 py-2 outline-none" value={selectedTerm} onChange={(e) => setSelectedTerm(e.target.value)}>
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-brand-50/30 dark:hover:bg-brand-900/20 transition-all group">
                      ...
                      <td className="px-6 py-5">
                        <span className="px-3 py-1 rounded-full text-[10px] font-bold uppercase bg-warning-100 dark:bg-warning-900/30 text-warning-700 dark:text-warning-400">Review</span>
                      </td>
                      <td className="px-8 py-5 text-right">
                        <button 
                          onClick={() => { setSelectedStudentId(s.id); setView("form"); }}
                          className="bg-brand-600 text-white px-5 py-2 rounded-xl text-xs font-bold hover:bg-brand-700 dark:hover:bg-brand-500 shadow-lg shadow-brand-200 dark:shadow-none transition-all active:scale-95"
                        >
                          Detail Report
                        </button>
                      </td>
          ...
          <div className="sm:hidden divide-y divide-gray-50 dark:divide-gray-700">
            {students.map((s) => {
              return (
                <div key={s.id} className="p-5 space-y-4 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                  ...
                  <button 
                    onClick={() => { setSelectedStudentId(s.id); setView("form"); }}
                    className="w-full bg-brand-600 text-white py-3 rounded-2xl text-xs font-bold hover:bg-brand-700 shadow-md active:scale-[0.98] transition-all"
                  >
                    Detail Report
                  </button>
                </div>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 print:m-0 print:shadow-none print:border-none">
          <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl dark:shadow-none border border-gray-100 dark:border-gray-700 overflow-hidden print:border-none print:shadow-none">
            {/* Form Header */}
            <div className="p-6 md:p-8 bg-brand-900 dark:bg-brand-950 text-white print:bg-white print:text-black print:border-b-2 print:border-black print:p-4">
              <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                <div>
                  <p className="text-brand-300 dark:text-brand-400 text-[10px] font-bold uppercase tracking-[0.2em] mb-1 print:text-gray-600">Mentor Review • {selectedClass} • {selectedTerm}</p>
            ...
            {/* Subject Selector for Mentor */}
            <div className="p-4 sm:p-6 bg-gray-50/50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-700 print:hidden overflow-x-auto">
              <div className="flex gap-2 min-w-max">
                {subjects.map((s) => (
                  <button
                    key={s.id}
                    onClick={() => setSelectedSubject(s.id)}
                    className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${selectedSubject === s.id ? 'bg-brand-600 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'}`}
                  >
                    {s.name}
                  </button>
                ))}
            ...
            <div className="print:block">
                ...
                <table className="w-full min-w-[600px]">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700 print:bg-white">
                      <th className="px-8 py-4 text-left text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest print:text-black print:px-4">Kriteria / Indikator Penilaian (Bisa Diedit Mentor)</th>
                      <th className="px-8 py-4 text-center text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest w-40 print:text-black print:px-4">Nilai dari Guru</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={c.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition group/row print:hover:bg-white">
                        ...
                        <td className="px-8 py-6 text-center align-middle print:px-4 print:py-4">
                          <input
                            ...
                            readOnly
                            placeholder="0.00"
                            className={`w-24 text-center py-3 border-2 dark:border-gray-700 rounded-2xl text-lg font-black outline-none cursor-not-allowed print:border-none print:text-black print:bg-white ${scoreColor(currentStudent?.grades[c.id] || "")}`}
                          />
                        </td>
                      </tr>
                    ))}
              ...
              <div className="sm:hidden p-4 space-y-6 print:hidden">
                {subject.criteria.map((c) => (
                  <div key={c.id} className="space-y-3 p-4 bg-gray-50/50 dark:bg-white/[0.02] rounded-2xl border border-gray-100 dark:border-white/5">
                    ...
                    <textarea
                      ...
                      className="w-full text-sm font-medium text-gray-700 dark:text-gray-200 leading-relaxed p-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl outline-none focus:ring-2 focus:ring-brand-500/20 transition-all"
                      rows={3}
                    />
                    <div className="flex items-center justify-between gap-4 pt-3 border-t border-gray-100 dark:border-gray-800">
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Nilai dari Guru</p>
                      <input
                        type="text"
                        inputMode="decimal"
                        value={currentStudent?.grades[c.id] || ""}
                        readOnly
                        placeholder="0.00"
                        className={`w-24 text-center py-2.5 border-2 dark:border-gray-700 rounded-xl text-lg font-black outline-none cursor-not-allowed ${scoreColor(currentStudent?.grades[c.id] || "")}`}
                      />
                    </div>
                  </div>
                ))}
                ...
                <button
                  onClick={() => addCriterion(subject.id)}
                  className="w-full py-4 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-2xl text-xs font-bold text-gray-400 hover:text-brand-500 hover:border-brand-500/50 transition-all bg-gray-50/50 dark:bg-transparent"
                >
                  + Tambah Kriteria Baru
                </button>

                <div className="bg-brand-600 dark:bg-brand-500 rounded-2xl p-6 text-white shadow-lg shadow-brand-200 dark:shadow-none flex justify-between items-center">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-70 mb-1">Average Score</p>
                    <p className="text-xs font-bold">{getLevelLabel(calcAverage(currentStudent!.grades, subject.criteria)).label}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-3xl font-black">{calcAverage(currentStudent!.grades, subject.criteria)}</p>
                  </div>
                </div>
              </div>
            </div>
